function [xhatnc, xhatc, xhatfir, numnc, dennc, numc, denc, thetahatfir] = est_add(x, v, N, Ahat, sigma2hat, Anoisehat, sigma2noisehat, SigmaYxhat, SigmaYYhat)
    ...
    sigma2v = var(v);
    MSE_fir = mean((xhatfir - x).^2) / sigma2v;
    MSE_nc = mean((xhatnc - x).^2) / sigma2v;
    MSE_c = mean((xhatc - x).^2) / sigma2v;
